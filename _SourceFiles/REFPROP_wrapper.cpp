...
// is called by props_REFPROP and satprops_REFPROP
    ...
    *RefpropdllInstance = LoadLibrary(DLL_PATH);
	if (!(int)*RefpropdllInstance){
		sprintf(errormsg,"Cannot open %s",DLL_PATH);	
		if (DEBUGMODE) printf("In init_REFPROP: %s\n",errormsg);
		return 1;
	}
	...
	switch(ierr){
		case 101:
			...
			if (DEBUGMODE) printf("init_REFPROP: Error 101\n");
			sprintf(errormsg,"error in opening file %s",hf);	
			break;
		case 102:
			if (DEBUGMODE) printf("init_REFPROP: Error 102\n");
			strcpy(errormsg,"error in file or premature end of file");	
			break;
		case -103:
			if (DEBUGMODE) printf("init_REFPROP: Error -103\n");
			strcpy(errormsg,"unknown model encountered in file");	
			break;
 		case 104:
			if (DEBUGMODE) printf("init_REFPROP: Error 104\n");
			strcpy(errormsg,"error in setup of model");	
			break;
		case 105:
			if (DEBUGMODE) printf("init_REFPROP: Error 105\n");
			strcpy(errormsg,"specified model not found");	
			break;
		case 111:
			if (DEBUGMODE) printf("init_REFPROP: Error 111\n");
			strcpy(errormsg,"error in opening mixture file");	
			break;
		case 112:
			if (DEBUGMODE) printf("init_REFPROP: Error 112\n");
			strcpy(errormsg,"mixture file of wrong type");	
			break;
		case 114:
			if (DEBUGMODE) printf("init_REFPROP: Error 114\n");
			strcpy(errormsg,"nc<>nc from setmod");	
			break;
		case 0:
		default:
			if (DEBUGMODE) printf("init_REFPROP: Error default(%i)\n",ierr);
...
//	double props[17];
	...
	if (DEBUGMODE) printf("starting props_REFPROP:\n");
	...
	if(props[0]=(double)init_REFPROP(fluidnames, REFPROP_PATH, &i, herr, &RefpropdllInstance, errormsg)){
		printf("Error(%i) initializing REFPROP: %s\n", props[0], errormsg);
		return 0;
	}
	ierr=0;
...
//			}
	...
	if (ierr==0){
		...
		/*    If q = 990 Then Quality = "Not calculated"
			  If q = 998 Then Quality = "Superheated vapor with T>Tc"
			  If q = 999 Then Quality = "Supercritical state (T>Tc, p>pc)"
			  If q = -998 Then Quality = "Subcooled liquid with p>pc"*/
	}
