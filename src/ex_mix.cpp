 ...
 * Description : example for Fortran and C++ interoperation. This file is
...
int main(int argc, char* argv[]) {
  if ( argc != 2 ) {
    printf ("usage: %s <path-to-fluid-files>\n", std::string(argv[0]).c_str());
    return 1;
  }
  ...
  //char* REFPROP_PATH_CHAR = "/path/to/fluids";
  char REFPROP_PATH_CHAR[filepathlength];
  strcpy(REFPROP_PATH_CHAR,std::string(argv[1]).c_str());
  ...
  if (!REFPROP_PATH.isDirectory()) REFPROP_PATH.makeDirectory(); //REFPROP_PATH.append(REFPROP_PATH.separator());
  ...
  bool is_fluids = ( 0 == Poco::icompare(REFPROP_PATH[REFPROP_PATH.depth()-1], FLUIDS_CHAR) );
  if (!is_fluids) FLD_PATH.pushDirectory(FLUIDS_CHAR);
  ...
  // 	if ( !theFile.isFile() || !theFile.canRead() ){
    ...
    if ( !theFile.exists() || !theFile.isDirectory() || !theFile.canRead() ){
